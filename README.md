...
Moreover, the existence of other variances, such as rotations, scaling or changes in illumination may reduce the similarity between fake test images and real ones. The utilization of test data (without labels) is authorized by transductive learning which is addressed in [2]. Incomplete supervision concerned the situation in which a small amount of labeled data and abundant unlabeled data are available, and the labeled data was insufficient to train a good learner. The unlabeled data are exploited by semi-supervised learning in addition to labeled data to improve learning performance. Transductive learning is a special type of semi-supervised learning and assumes that the available unlabeled data is exactly the test data.
